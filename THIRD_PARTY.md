...
> **版权行来历与确认状态。** `LICENSE` 里的版权行写的是 `hyp13048196060`。
> 它**不是查证来的**:`package.json` 里没有 `author` 字段,执行时只能取本仓库的
> git 提交者身份来填。因此它最初是个占位值,并在本文件里标为「待确认」。
>
> **2026-09-18,仓库所有者本人在转公开前确认了这一行,决定维持不变。**
> 也就是说:这一行的**权威性来自所有者本人的确认**,不是来自任何查证。
> 若日后要换署名,直接改 `LICENSE` 第一行即可,别处没有引用它。
>
> 之所以把这段过程留着而不删掉:它是「占位值必须显式标出来、并且在造成对外
> 影响之前被确认」这条规矩的执行记录。删掉过程只留下结论,下一个人就不知道
> 这行曾经是个占位,也就不会在换署名时想到要回来同步这里。
...
- 当前子集:两个文件各约 344 KB(源字体 24.4 MB → 缩小约 70 倍)。
  规模有两个**不同口径**的数字,别看混了(实读自产物 `name`/`maxp`/`cmap` 表):
  - **1697** —— `maxp.numGlyphs`,字形表条目数。含 `.notdef` 与只能经 GSUB
    取到的字形,不全是能直接打出来的字。
  - **1670** —— `cmap` 里可映射的码位数,其中汉字 1485。
    这才是"界面上能打出多少个不同的字"的量。

  **校验界面用字覆盖,要用 1670 这一侧。**用 1697 去比会高估可打出的字数。
...
- **本项目版权人**(第 1 节):**未经查证,但经所有者确认**。值取自 git 身份,
  由仓库所有者本人在转公开前确认维持。它属于"署名选择"而非"可核实的事实",
  所以这里不写"已核实",只写"已确认"。
